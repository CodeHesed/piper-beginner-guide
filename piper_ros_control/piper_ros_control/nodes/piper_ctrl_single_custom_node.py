...
# Customly edited for speed adjustment
import rclpy
...
class PiperRosNode(Node):
    ...
    def __init__(self) -> None:
        super().__init__('piper_ctrl_single_custom_node')
        # ROS parameters
        ...
        # Joint (添加 joint7/joint8 用于 URDF gripper 手指)
        self.joint_states = JointState()
        self.joint_states.name = ['joint1', 'joint2', 'joint3', 'joint4', 'joint5', 'joint6', 'gripper', 'joint7', 'joint8']
        self.joint_states.position = [0.0] * 9
        self.joint_states.velocity = [0.0] * 9
        self.joint_states.effort = [0.0] * 9
        ...
        self.joint_states_feedback.position = [0.0] * 9
        self.joint_states_feedback.velocity = [0.0] * 9
        self.joint_states_feedback.effort = [0.0] * 9
        # Joint ctrl
        self.joint_ctrl = JointState()
        self.joint_ctrl.name = ['joint1', 'joint2', 'joint3', 'joint4', 'joint5', 'joint6', 'gripper', 'joint7', 'joint8']
        self.joint_ctrl.position = [0.0] * 9
        self.joint_ctrl.velocity = [0.0] * 9
        self.joint_ctrl.effort = [0.0] * 9
        # Enable flag
        ...
        # Custom features
        self.create_subscription(Int32, 'move_speed', self.move_speed_callback, 1)
        self.move_speed = 50 # 50 is default - which means 50% of max speed
        self.MAX_SPEED = 100
        self.MIN_SPEED = 1
    ...
    def PublishArmJointAndGripper(self):
        ...
        # joint7/joint8 是两个手指关节，从 gripper 角度计算
        joint_7: float = joint_6 / 2  # 左手指
        joint_8: float = -joint_6 / 2  # 右手指
        self.joint_states.position = [joint_0, joint_1, joint_2, joint_3, joint_4, joint_5, joint_6, joint_7, joint_8]
        self.joint_states.velocity = [vel_0, vel_1, vel_2, vel_3, vel_4, vel_5, 0.0, 0.0, 0.0]
        self.joint_states.effort = [effort_0, effort_1, effort_2, effort_3, effort_4, effort_5, effort_6, 0.0, 0.0]

        self.joint_states_feedback.position = [joint_0, joint_1, joint_2, joint_3, joint_4, joint_5, joint_6, joint_7, joint_8]
        self.joint_states_feedback.velocity = [vel_0, vel_1, vel_2, vel_3, vel_4, vel_5, 0.0, 0.0, 0.0]
        self.joint_states_feedback.effort = [effort_0, effort_1, effort_2, effort_3, effort_4, effort_5, effort_6, 0.0, 0.0]
        self.joint_states_feedback.header.stamp = self.joint_states.header.stamp
        # 发布所有消息 (只检查前6个关节的范围)
        if any(abs(pos) > 3.5 for pos in self.joint_states_feedback.position[:6]):
    ...
    def PublishArmCtrlAndGripper(self):
        ...
        joint_7: float = joint_6 / 2  # 左手指
        joint_8: float = -joint_6 / 2  # 右手指
        self.joint_ctrl.position = [joint_0, joint_1, joint_2, joint_3, joint_4, joint_5, joint_6, joint_7, joint_8]
        # 只检查前6个关节的范围
        if any(abs(pos) > 3.5 for pos in self.joint_ctrl.position[:6]):
            self.get_logger().warn("Joint state abnormal: value exceeds ±3.5 rad")
        else:
    ...
    def pos_callback(self, pos_data):
        ...
        if(self.GetEnableFlag()):
            self.piper.MotionCtrl_2(0x01, 0x00, self.move_speed)
        ...
        if self.GetEnableFlag():
            ...
            if not all_zeros:
                ...
                if lens == 7:
                    ...
                    self.piper.MotionCtrl_2(0x01, 0x01, self.move_speed)
            else:
                self.piper.MotionCtrl_2(0x01, 0x01, self.move_speed)
    ...
    # Custom feature
    def move_speed_callback(self, msg):
        self.get_logger().info(f"Received move speed set: {msg.data}")
        _move_speed = msg.data
        self.move_speed = max(self.MIN_SPEED, min(self.MAX_SPEED, _move_speed))
        if (self.move_speed != _move_speed):
            self.get_logger().warn(f"Input speed out of range. Adjusting into range: {self.move_speed}")
